...
with open(datafile, 'a+',newline='',encoding='utf-8') as csvfile:
    ...
    if len(csvread) == 0:
        fields = ['ID','Name','Mobile Number','Date of Birth','Address 1',
                  'Address 2','State','Guardian\'s Name','Email ID','College',
...
def submitWindow():
    ...
    #checkbox
    cAgree=IntVar()
    cag = ttk.Checkbutton(top, text="All the informations furnished above are " \
                                 "correct", variable=cAgree)
    ...
    def checkValues():
        ...
        #check email
        data=str(eMail.get())
        if (not re.search(r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+'
        r'(\.[A-Z|a-z]{2,})+', data)):
    ...
    def click():
        ...
        with open(datafile,'a',newline='',encoding='utf-8') as csvfile:
            ...
            datarow=[uid,eName.get(),eMnum.get(),eDOB.get(),eAd1.get(),
                eAd2.get(),eState.get(),eGname.get(),eMail.get(),eNcol.get(),
                eCname.get(),eDept.get(),eyr.get()]
            #write data in file
